...
class GAV:
    def __init__(self, group_id: str, artifact_id: str, version: str, scope="", packaging="", classifier="", typ=""):
...
def get_available_versions(g: str, a: str, use_remote=False, max_num=None) -> list[str]:
    ...
    if response.headers["Content-Type"] != "text/xml" and not use_remote:
...
def get_scm_from_pom(pom_response: requests.Response, g: str, a: str, v: str):
    ...
    try:
        ...
        print(e)
        print(f"Skipping, resource is not in valid XML format")
        raise PomNotFoundException(f"Could not find the pom of: {g}:{a}:{v}")
...
def unroll_range(range_str: str, available_versions: list[str]) -> list[str]:
    """Given a range or concatenated ranges, use available_versions to unroll into a list of versions."""
    # Normalize the input: Remove leading and trailing brackets for easier parsing
    range_str = range_str.strip()
    if range_str.startswith("[") and range_str.endswith("]"):
        range_str = range_str[1:-1]

    # Split concatenated ranges by separating on "],["
    ranges = range_str.split("],[")

    def process_single_range(single_range: str) -> list[str]:
        bounds = single_range.split(",")
        if len(bounds) == 1:
            # Case: Single version, e.g., [6.2.0]
            lowerbound = bounds[0]
            upperbound = bounds[0]
        elif len(bounds) == 2:
            # Case: Range with lower and upper bounds, e.g., [6.0.0,6.1.15]
            lowerbound, upperbound = bounds
        else:
            raise ValueError(f"Invalid range format: {single_range}")
    ...
    for single_range in ranges:
        all_versions = process_single_range(single_range.strip()) + all_versions
