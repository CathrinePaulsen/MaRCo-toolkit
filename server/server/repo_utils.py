...
def repo_has_tests(repo_path: Path) -> bool:
    logger.debug(f"Checking runnable tests for {repo_path}")
...
def compile_repo(repo_path: Path, use_pom="pom.xml", save_as=None) -> bool:
    """Note: test-compile will compile both source and test code, as opposed to compile which only compiles source.
    Spotless and Spotbugs checks are typically done during verify phase, and not the test-compile phase, but may still
    be configured by some projects to run during test-compile so we add the skip flags."""
    logger.debug(f"Compiling repo {repo_path}, save log as {save_as}")
    assert Path.is_dir(repo_path)
    old_dir = os.getcwd()
    os.chdir(repo_path)
