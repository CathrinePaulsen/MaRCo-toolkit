...
### Structure
Each directory is an installable Python package and contains its own `README.md` with more detailed documentation and run instructions.
...
* `demo`: contains a follow-along demo of how you can use MaRCo to solve version conflicts (diamond dependencies)

#### Miscellaneous data and evaluation
* `data`: contains the dataset of compatible Dependabot updates, and 323 reusable MaRCo-generated compatibility mappings.
* `evaluatelinker`: contains the scripts and data used to evaluate the GitHub linking algorithm against
  [Reproducible Central](jvm-repo-rebuild/reproducible-central) as ground truth.

Each module contains its own `README.md` containing more detailed documentation and run instructions.

### Setup
To install the full MaRCo toolkit, first set your variables in `core/config.py`, then run:
```
pip install core/
pip install server/
pip install client/
...
#### Docker
We provide a `Dockerfile` which you can use to run MaRCo in Docker containers.
After building the image, you can run `docker compose up` to spin up the containers necessary to run MaRCo.
To mount an existing .m2 folder, please specify your .m2 path in `compose.yaml`.
